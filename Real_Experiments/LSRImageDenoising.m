...
function LSRImageDenoising(monte_carlos,path_to_rand_state,path_to_pic, image, randState)
    ...
    % SeDiL Parameters
    paramSeDiL = init_learning_parameters_dict();
    paramSeDiL.max_iter = 1000;
    paramSeDiL.mu = 100;
    paramSeDiL.lambda = 0.1/p;
    paramSeDiL.kappa =0.1/p;
    paramSeDiL.q = [0,1];
    paramSeDiL.d_sz = M;
    paramSeDiL.verbose = 1100; %iteration at which display results
    paramSeDiL.Sp_type = 'LogSquare';
    ...
    for mont_crl = 1:N_montcarl
        ...
        for sigma_ind=1:2
            ...
            %SeDiL Y Tensor
            Y_tns_SeDiL = reshape(Y_train_SeDiL,M(1),M(2),M(3),N);
            Y_tns = reshape(Y_train,M(1),M(2),M(3),N);
            
            for k=1:3
                D_initk = unfold(Y_tns,size(Y_tns),k);
                D_initk_SeDiL = unfold(Y_tns_SeDiL,size(Y_tns_SeDiL),k);
                
                cols_k = randperm(N*m/M(k),P(k));
                
                D_init_k{k} = normc(D_initk(:,cols_k));
                D_init_k_SeDiL{k} = normc(D_initk_SeDiL(:,cols_k));
            end
            ...
            Rep_err_test_TeFDiL_OMP(mont_crl,sigma_ind)=norm(reshape(input_data-Image_out_TeFDiL_OMP./cnt_TeFDiL,dim1,dim2*N_freq),'fro')^2/numel(input_data);
             ...
             %% SeDiL
            disp('Training structured dictionary using SeDiL')
            paramSeDiL.D{1}     = D_init_k_SeDiL{1};
            paramSeDiL.D{2}     = D_init_k_SeDiL{2};
            paramSeDiL.D{3}     = D_init_k_SeDiL{3};
            tic
            Learn_para_SeDiL = learn_separable_dictionary(Y_tns_SeDiL, paramSeDiL);
            toc
            ...
            %%%Dictionary Representation training Error
            Rep_err_train_SEDIL(mont_crl,sigma_ind) = norm(Y_train_SeDiL - D_SEDIL*X_train_SEDIL,'fro')^2/numel(Y_train_SeDiL);

            Y_SEDIL = D_SEDIL*X_train_SEDIL;
            %%%Reconstructing the image from the overlapping patches
            [Image_out_SEDIL, cnt_SEDIL] = ImageRecon2(input_data,N_blocks,dim2_block,step,patch_size,Y_SEDIL,N_freq);
